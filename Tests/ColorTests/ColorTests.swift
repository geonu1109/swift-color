...
/// A minimal user-defined space (a single gray value) used to test that custom
/// `ColorRepresentation`s are stored and read back exactly through `Color`.
private struct GraySpace: ColorRepresentation {
    var value: Double
    var alpha: Double = 1
    func toXYZ() -> XYZ { SRGB(red: value, green: value, blue: value, alpha: alpha).toXYZ() }
    static func fromXYZ(_ xyz: XYZ) -> GraySpace {
        let s = SRGB.fromXYZ(xyz)
        return GraySpace(value: (s.red + s.green + s.blue) / 3, alpha: xyz.alpha)
    }
}
...
struct StorageTests {
    ...
    @Test("space reports the color space the value is stored in")
    func authoredSpace() {
        #expect(Color(SRGB.red).space == .sRGB)
        #expect(Color(hex: "#3B82F6")!.space == .sRGB)
        #expect(Color(OKLCH(lightness: 0.7, chroma: 0.1, hue: 200)).space == .oklch)
        #expect(Color(DisplayP3(red: 1, green: 0, blue: 0)).space == .displayP3)
    }
}
...
struct ConversionTests {
    ...
    @Test("Custom ColorRepresentation conformance interoperates")
    func customSpace() {
        // Any representation conforming to ColorRepresentation converts via XYZ for free.
    ...
    @Test("Custom representation is stored as-is, read back exactly, and Hashable")
    func customStorage() throws {
        let gray = GraySpace(value: 0.4, alpha: 0.8)
        let color = Color(gray)

        // Stored as a custom space, and read back exactly (no XYZ round-trip).
        #expect(color.space == .custom)
        #expect(color.converted(to: GraySpace.self) == gray)

        // Hashable: equal customs compare equal, different ones don't.
        #expect(Color(GraySpace(value: 0.4, alpha: 0.8)) == color)
        #expect(Color(GraySpace(value: 0.5, alpha: 0.8)) != color)

        // Codable degrades a custom space to XYZ (color preserved, tag becomes .xyz).
        let decoded = try JSONDecoder().decode(Color.self, from: JSONEncoder().encode(color))
        #expect(decoded.space == .xyz)
        #expect(isClose(decoded.srgb.red, color.srgb.red))
    }
}
...
struct CodableBridgingTests {
    ...
    @Test("Codable JSON shape: colorSpace tag, nested components, top-level alpha")
    func codableShape() throws {
        let enc = JSONEncoder(); enc.outputFormatting = [.sortedKeys]
        let json = String(decoding: try enc.encode(Color(DisplayP3(red: 1, green: 0, blue: 0, alpha: 0.5))), as: UTF8.self)
        #expect(json == #"{"alpha":0.5,"colorSpace":"display-p3","components":{"blue":0,"green":0,"red":1}}"#)
    }
...
@Suite("Metrics & edge-case robustness")
struct MetricsTests {
    @Test("alpha is clamped to 0...1")
    func alphaClamp() {
        #expect(Color(SRGB.red).alpha(2.0).alpha == 1)
        #expect(Color(SRGB.red).alpha(-1).alpha == 0)
        #expect(isClose(Color(SRGB.red).alpha(0.3).alpha, 0.3))
    }

    @Test("Achromatic colors have hue 0, not float noise")
    func achromaticHue() {
        #expect(isClose(Color(SRGB.white).oklch.hue, 0))
        #expect(isClose(Color(SRGB.gray).oklch.hue, 0))
        #expect(isClose(Color(SRGB.black).oklch.hue, 0))
    }

    @Test("WCAG luminance and contrast ratio")
    func contrast() {
        #expect(isClose(Color(SRGB.black).contrastRatio(to: Color(SRGB.white)), 21, tolerance: 0.05))
        #expect(isClose(Color(SRGB.white).contrastRatio(to: Color(SRGB.white)), 1))
        #expect(Color(SRGB.white).relativeLuminance > Color(SRGB.black).relativeLuminance)
    }

    @Test("Perceptual difference (ΔEOK)")
    func difference() {
        #expect(isClose(Color(SRGB.red).difference(to: Color(SRGB.red)), 0))
        #expect(Color(SRGB.red).difference(to: Color(SRGB.green)) > 0.2)
    }

    @Test("Mixing blends perceptually and takes the short hue arc")
    func mixing() {
        let mid = Color(OKLCH.black).mixed(with: Color(OKLCH.white), amount: 0.5, in: .oklab)
        #expect(isClose(mid.oklab.lightness, 0.5, tolerance: 0.01))
        let a = Color(OKLCH(lightness: 0.6, chroma: 0.2, hue: 350))
        let b = Color(OKLCH(lightness: 0.6, chroma: 0.2, hue: 10))
        let h = a.mixed(with: b, amount: 0.5, in: .oklch).oklch.hue
        #expect(isClose(h, 0, tolerance: 0.5) || isClose(h, 360, tolerance: 0.5))
    }
}
